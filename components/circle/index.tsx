...
export default function Circle({
  ...
  const pathString = useMemo(() => {
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      const y =
        b1 * Math.sin(t) * Math.exp(sParameter * x) * currentCompression;

      const rotatedX = x * Math.cos(angleRad) - y * Math.sin(angleRad);
      const rotatedY = x * Math.sin(angleRad) + y * Math.cos(angleRad);

      points.push([rotatedX * scale + centerX, rotatedY * scale + centerY]);
    }
  ...
  return (
    <div className="relative h-[300px] w-[300px] overflow-visible mx-auto">
      {activeDot && (
        <TitleDisplay isEgg={isEgg} angle={angle} dot={activeDot} />
      )}
